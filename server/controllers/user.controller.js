...
const { userCritSelector, postAuthorSelector, quoteSelector } = require("../helpers/selectors");
...
const getProfileTimeline = asyncHandler(async (req, res, next) => {
    ...
    const pipeline = [
        {
            $match: {
                $and: [
                    { author: user._id },
                    { $or: [{ recrits: { $in: [user._id] } }, { replyTo: { $eq: null } }] },
                ],
            },
        },
    ...
    const response = await paginate(Crit, pipeline, req.pagination);
