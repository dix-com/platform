...
const getUser = asyncHandler(async (req, res, next) => {
    ...
    const critsCount = await Crit.countDocuments({
        author: user._id,
    });

    const likesCount = await Crit.countDocuments({
        likes: {
            $in: [user._id],
        },
    });

    const mediaCount = await Crit.countDocuments({
        author: user._id,
        media: {
            $ne: [],
        },
...
const getCrits = asyncHandler(async (req, res, next) => {
    const { username } = req.params;
    ...
    if (!user) {
        return next(
            new NotFoundError(
                "Can't retrieve crits since the user does not exist!"
            )
        );
    }

    const crits = await Crit.find({ author: user._id })
        .populate("author")
        .sort({ createdAt: -1 });
...
const getLikedCrits = asyncHandler(async (req, res, next) => {
    const { userId } = req.params;

    const likedCrits = await Crit.find({ likes: { $in: [userId] } })
        .populate("author")
        .sort({
            createdAt: -1,
        });

    // TODO: Trim the liked crits?
    console.log(likedCrits);
